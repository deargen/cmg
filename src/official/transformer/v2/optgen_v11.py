...
def get_valnet(params, inputs):
    ids = tf.keras.backend.argmax(inputs, axis=-1)

    embedding_softmax_layer = embedding_layer.EmbeddingFreezable(
    ...
    z = tf.keras.layers.Bidirectional(
        tf.keras.layers.LSTM(params["valnet_hidden_size"], name="prop_lstm", trainable=False)
    )(embedded_inputs)
    # z = tf.stop_gradient(z)
    outputs = tf.keras.layers.Dense(1, use_bias=True, activation=tf.nn.sigmoid, name="output", trainable=False)(z)
    # outputs = tf.stop_gradient(outputs)

    return outputs


def get_propnet(params, inputs):
    ids = tf.keras.backend.argmax(inputs, axis=-1)
    ...
    z = tf.keras.layers.Bidirectional(
        tf.keras.layers.LSTM(params["propnet_hidden_size"], name="propnet_lstm", trainable=False)
    )(embedded_inputs)
    z = tf.keras.layers.Dense(100, use_bias=True, activation=tf.nn.sigmoid, name="propnet_idense", trainable=False)(z)
    outputs = tf.keras.layers.Dense(3, use_bias=True, activation=None, name="propnet_output", trainable=False)(z)
