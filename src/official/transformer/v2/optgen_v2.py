...
# ==============================================================================
"""Defines the OptimizedMoleculeGenerator model in TF 2.0.
    ...
    with tf.io.gfile.GFile(json_file, "r") as reader:
        ...
        for (key, value) in six.iteritems(json_object):
            if value == "None":
                ...
                params[key] = False
            # if key in ['qkv_bias', 'enable_metrics_in_training']:
...
class OptimizedMoleculeGenerator(tf.keras.Model):
    """OptimizedMoleculeGenerator model with Keras.

    Implemented as described in: https://arxiv.org/pdf/1706.03762.pdf

    The OptimizedMoleculeGenerator model consists of an encoder and decoder. The input is an int
    ...
    def __init__(self, params, name=None):
        """Initialize layers to build OptimizedMoleculeGenerator model.

        Args:
          params: hyperparameter object defining layer sizes, dropout values, etc.
          name: name of the model.
        """
        super(OptimizedMoleculeGenerator, self).__init__(name=name)
        self.params = params
        self.embedding_softmax_layer = embedding_layer.EmbeddingSharedWeights(
            params["vocab_size"], params["hidden_size"])
        self.positional_emb = embedding_layer.PositionalEmbeddingSharedWeights(
            params["max_length"], params["hidden_size"], params["relu_dropout"])
        ...
        #     self.pos_embedding_weights = self.add_weight(
        #         "pos_weights",
        #         shape=[params["max_length"], params["hidden_size"]],
        #         dtype="float32",
    ...
    def encode(self, inputs, attention_bias, training):
        ...
        with tf.name_scope("encode"):
            ...
            encoder_inputs = self.positional_emb(embedded_inputs)
            ...
            # if training:
            #     encoder_inputs = tf.nn.dropout(encoder_inputs, rate=self.params["attention_dropout"])
            ...
            # with tf.name_scope("add_pos_encoding"):
            #     length = tf.shape(embedded_inputs)[1]
            #     position_embeddings = tf.slice(self.pos_embedding_weights, [0, 0],
            #                                    [length, -1])
            #     num_dims = len(embedded_inputs.shape.as_list())
            #
            ...
            #     position_broadcast_shape.extend([length, self.hidden_size])
            #     position_embeddings = tf.reshape(position_embeddings,
            #                                      position_broadcast_shape)
            #     encoder_inputs = embedded_inputs + position_embeddings
            #
            #     encoder_inputs = self.layer_norm(encoder_inputs)
            ...
            else: # TODO: this is work-around for buggy model
                encoder_inputs = tf.nn.dropout(
                    encoder_inputs, rate=0.000000000001)
    ...
    def decode(self, targets, encoder_outputs, attention_bias, training):
        ...
        with tf.name_scope("decode"):
            ...
            decoder_inputs = self.positional_emb(decoder_inputs)
            decoder_inputs = tf.cast(decoder_inputs, self.params["dtype"])
    ...
    def predict(self, encoder_outputs, encoder_decoder_attention_bias, training):
        ...
        initial_ids = tf.ones([batch_size], dtype=tf.int32)  # 1: [CLS]
        # initial_ids = tf.fill([batch_size], 1)  # 2: [BEGIN]
        ...
        top_decoded_ids = decoded_ids[:, 0, 0:] # should include [CLS] [BEGIN]
        top_scores = scores[:, 0]
...
class EncoderStack(tf.keras.layers.Layer):
    """OptimizedMoleculeGenerator encoder stack.
    ...
    def build(self, input_shape):
        ...
        for _ in range(params["num_hidden_layers"]):
            ...
            self_attention_layer = attention_layer.SelfAttention(
                ...
                params["attention_dropout"], use_bias=params["qkv_bias"])
...
class DecoderStack(tf.keras.layers.Layer):
    """OptimizedMoleculeGenerator decoder stack.
