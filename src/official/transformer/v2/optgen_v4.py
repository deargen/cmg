...
def create_model(params, is_train):
    ...
    with tf.name_scope("model"):
        if is_train:
            ...
            logits_py, logits_px = internal_model([inputs, px, py, targets], training=is_train)
            # logits = internal_model([inputs, px, sim, py, targets], training=is_train)
            vocab_size = params["vocab_size"]
            label_smoothing = params["label_smoothing"]
            if params["enable_metrics_in_training"]:
                logits_py = metrics.MetricLayer(vocab_size)([logits_py, targets])
            logits_py = tf.keras.layers.Lambda(lambda x: x, name="logits_py")(logits_py)
            logits_px = tf.keras.layers.Lambda(lambda x: x, name="logits_px")(logits_px)
            model = tf.keras.Model([inputs, px, py, targets], logits_py)
            # model = tf.keras.Model([inputs, px, sim, py, targets], logits)
            loss_py = metrics.transformer_loss(
                logits_py, targets, label_smoothing, vocab_size)
            loss_px = metrics.transformer_loss(
                logits_px, inputs, label_smoothing, vocab_size)
            model.add_loss(loss_py)
            model.add_loss(loss_px)
            return model
...
class Transformer(tf.keras.Model):
    ...
    def call(self, inputs, training):
        ...
        with tf.name_scope("Transformer"):
            ...
            encoder_outputs_px = self.concat_property(encoder_outputs, px-px)
            encoder_outputs_py = self.concat_property(encoder_outputs, py-px)

            # Generate output sequence if targets is None, or return logits if target
            # sequence is known.
            if targets is None:
                return self.predict(encoder_outputs_py, attention_bias, training)
            else:
                logits_py = self.decode(targets, encoder_outputs_py, attention_bias, training)
                logits_px = self.decode(inputs, encoder_outputs_px, attention_bias, training)
                return logits_py, logits_px

    def concat_property(sefl, encoder_outputs, property):
    # def concat_property(sefl, encoder_outputs, px, sim, py):
        ...
        property = tf.tile(tf.expand_dims(property, axis=1), multiples=[1, input_length, 1])
        result = tf.concat([encoder_outputs, property], axis=-1)
