...
class Transformer(tf.keras.Model):
    ...
    # def concat_property(sefl, encoder_outputs, px, sim, py):
        ...
        # xp_list = df[['X_logP', 'X_qed', 'X_drd2']].values, so only drd2
        px = px[:, 2]
        py = py[:, 2]

        px_e = tf.expand_dims(tf.expand_dims(px, axis=1), axis=1)
        px = tf.tile(px_e, multiples=[1, input_length, 1])
        # sim = tf.tile(tf.expand_dims(sim, axis=1), multiples=[1, input_length, 1])

        py_e = tf.expand_dims(tf.expand_dims(py, axis=1), axis=1)
        py = tf.tile(py_e, multiples=[1, input_length, 1])
