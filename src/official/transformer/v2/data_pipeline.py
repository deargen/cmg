...
from src.official.transformer.v2 import misc
from src.official.utils.misc import model_helpers
...
def _parse_example(serialized_example):
    ...
    data_fields = {
        ...
        "xp": tf.io.FixedLenFeature([3], tf.float32),
        # "sim": tf.io.FixedLenFeature([1], tf.float32),
        "yp": tf.io.FixedLenFeature([3], tf.float32),
...
def _batch_examples(dataset, batch_size, max_length):
    ...
    # def example_to_bucket_id(example_input, px, sim, py, example_target):
    def example_to_bucket_id(example_input, px, py, example_target):
    ...
    def batching_fn(bucket_id, grouped_dataset):
        ...
        # return grouped_dataset.padded_batch(bucket_batch_size, ([None], [3], [1], [3], [None]))
        return grouped_dataset.padded_batch(bucket_batch_size, ([None], [3], [3], [None]))
...
def _read_and_batch_from_files(
    ...
    # dataset = dataset.filter(lambda x, xp, sim, yp, y: _filter_max_length((x, y), max_length))
    dataset = dataset.filter(lambda x, xp, yp, y: _filter_max_length((x, y), max_length))
...
# def map_data_for_transformer_fn(x, px, sim, py, y):
def map_data_for_optgen_fn(x, px, py, y):
    ...
    if misc.is_v2():
        ...
        # return ((x, px, sim, py, y), )
        return ((x, px, py, y),)
    else:
        # For TF v1, Keras requires a dummy placeholder as the 2nd parameter.
        # return ((x, px, sim, py, y), tf.constant(0.0))
        return ((x, px, py, y), tf.constant(0.0))


# def map_data_for_transformer_fn(x, px, sim, py, y):
def map_data_for_transformer_fn(x, px, py, y):
    """Maps data for training, and handles weried behaviors for different vers."""
    # Will transform input x and targets y into tuple(x, y) as new model inputs.
    if misc.is_v2():
        # For TF v2, the 2nd parameter is omitted to make Keras training work.
