...
def get_valnet_with_id(ids, embedding_softmax_layer, lstm_layer, output_layer):
...
def get_valnet(logits, embedding_softmax_layer, lstm_layer, output_layer):
    ...
    return get_valnet_with_id(ids, embedding_softmax_layer, lstm_layer, output_layer)


def get_propnet_with_id(ids, embedding_softmax_layer, lstm_layer, idense_layer, output_layer):
    embedded_inputs = embedding_softmax_layer(ids)
    z = lstm_layer(embedded_inputs)
    z = idense_layer(z)
    outputs = output_layer(z)
...
def get_propnet(logits, embedding_softmax_layer, lstm_layer, idense_layer, output_layer):
    ids = tf.keras.backend.argmax(logits, axis=-1)
    return get_propnet_with_id(ids, embedding_softmax_layer, lstm_layer, idense_layer, output_layer)


def get_simnet_with_id(ids1, ids2, embedding_softmax_layer, lstm_layer, idense_layer, output_layer):
...
def get_simnet(logit1, inputs, embedding_softmax_layer, lstm_layer, idense_layer, output_layer):
    ...
    ids2 = inputs
    return get_simnet_with_id(ids1, ids2, embedding_softmax_layer, lstm_layer, idense_layer, output_layer)


def get_aux_layers(params):
    ...
    simnet_embedding_softmax_layer, simnet_bi_lstm_layer, simnet_idense_layer, simnet_output_layer \
        = None, None, None, None, None, None, None, None, None, None, None

    if params["use_valnet"]==1:
        valnet_embedding_softmax_layer = embedding_layer.EmbeddingFreezable(
            params["vocab_size"], params["valnet_hidden_size"], trainable=False, name="valnet_emb")
        valnet_bi_lstm_layer = tf.keras.layers.Bidirectional(
            ...
            name="valnet_bidirectional")
        # valnet_bi_lstm_layer = tf.keras.layers.Bidirectional(
        #     tf.keras.layers.LSTM(params["valnet_hidden_size"], name="valnet_lstm", trainable=False,
        #                          activation='sigmoid'), name="valnet_bidirectional")
    ...
    if params["use_propnet"]==1:
        propnet_embedding_softmax_layer = embedding_layer.EmbeddingFreezable(
            params["vocab_size"], params["propnet_hidden_size"], trainable=False, name="propnet_emb")
        propnet_bi_lstm_layer = tf.keras.layers.Bidirectional(
            ...
            name="propnet_bidirectional")
        # propnet_bi_lstm_layer = tf.keras.layers.Bidirectional(
        #     tf.keras.layers.LSTM(params["propnet_hidden_size"], name="prop_lstm", trainable=False,
        #                          activation='sigmoid'), name="propnet_bidirectional")
    ...
    if params["use_simnet"] == 1:
        simnet_embedding_softmax_layer = embedding_layer.EmbeddingFreezable(
            params["vocab_size"], params["simnet_hidden_size"], trainable=False, name="simnet_emb")
        simnet_bi_lstm_layer = tf.keras.layers.Bidirectional(
            ...
            name="simnet_bidirectional")
        # simnet_bi_lstm_layer = tf.keras.layers.Bidirectional(
        #     tf.keras.layers.LSTM(params["simnet_hidden_size"], name="simnet_lstm", trainable=False,
        #                          activation='sigmoid'), name="simnet_bidirectional")
    ...
    layers = (
    valnet_embedding_softmax_layer, valnet_bi_lstm_layer, valnet_output_layer, propnet_embedding_softmax_layer,
    propnet_bi_lstm_layer, propnet_idense_layer, propnet_output_layer, simnet_embedding_softmax_layer,
    simnet_bi_lstm_layer, simnet_idense_layer, simnet_output_layer)
...
def create_model(params, is_train):
    ...
    with tf.name_scope("model"):
        if is_train:
            ...
            scores = ret["scores"]
            if params["use_valnet"] == 1:
                aaa = get_valnet_with_id(inputs, valnet_embedding_softmax_layer, valnet_bi_lstm_layer, valnet_output_layer)
                aaa = tf.keras.layers.Lambda(lambda x: tf.reduce_sum(x, axis=-1), name="dummy1")(aaa)
                scores += aaa
            if params["use_propnet"] == 1:
                bbb = get_propnet_with_id(inputs, propnet_embedding_softmax_layer, propnet_bi_lstm_layer,
                                          propnet_idense_layer, propnet_output_layer)
                bbb = tf.keras.layers.Lambda(lambda x: tf.reduce_sum(x, axis=-1), name="dummy2")(bbb)
                scores += bbb
            if params["use_simnet"] == 1:
                ccc = get_simnet_with_id(inputs, inputs, simnet_embedding_softmax_layer, simnet_bi_lstm_layer,
                                        simnet_idense_layer, simnet_output_layer)
                ccc = tf.keras.layers.Lambda(lambda x: tf.reduce_sum(x, axis=-1), name="dummy3")(ccc)
                scores += ccc
...
class Transformer(tf.keras.Model):
    ...
    def __init__(self, params, aux_layers, name=None):
        ...
        self.valnet_embedding_softmax_layer, self.valnet_bi_lstm_layer, self.valnet_output_layer, \
        self.propnet_embedding_softmax_layer, self.propnet_bi_lstm_layer, \
        self.propnet_idense_layer, self.propnet_output_layer, \
        self.simnet_embedding_softmax_layer, self.simnet_bi_lstm_layer, \
        self.simnet_idense_layer, self.simnet_output_layer = aux_layers

        self.use_valnet = True
        self.use_propnet = True
        self.use_simnet = True

        if self.valnet_output_layer is None:
            self.use_valnet = False

        if self.propnet_output_layer is None:
            self.use_propnet = False

        if self.simnet_output_layer is None:
            self.use_simnet = False

        if params["use_beam_filter"]==1:
            self.use_beam_filter = True
        else:
            self.use_beam_filter = False

        if self.use_valnet==False and self.use_propnet==False and self.use_simnet==False:
            self.use_beam_filter = False
    ...
    def call(self, inputs, training):
        ...
        with tf.name_scope("Transformer"):
            ...
            # sequence is known.
            if targets is None:
                return self.predict(inputs, py, encoder_outputs, attention_bias, training)
            else:
    ...
    def predict(self, inputs, py, encoder_outputs, encoder_decoder_attention_bias, training):
        ...
        for i in range(self.params["beam_size"]):
            ...
            if self.use_propnet:
                prop_hat = get_propnet_with_id(candidate_ids, self.propnet_embedding_softmax_layer,
                                               self.propnet_bi_lstm_layer, self.propnet_idense_layer,
                                               self.propnet_output_layer)
                prop_diff = tf.math.abs(prop_hat-py)
                qed_diff = prop_diff[:, 1]
                drd2_diff = prop_diff[:, 2]
                beam_score_list.append((1-qed_diff) + (1-drd2_diff) )

            if self.use_simnet:
                sim_hat = get_simnet_with_id(candidate_ids, inputs, self.simnet_embedding_softmax_layer,
                                               self.simnet_bi_lstm_layer, self.simnet_idense_layer,
                                               self.simnet_output_layer)
                sim_hat = tf.squeeze(sim_hat, axis=-1)
                beam_score_list.append(sim_hat)

            if self.use_valnet:
                val_hat = get_valnet_with_id(candidate_ids, self.valnet_embedding_softmax_layer,
                                               self.valnet_bi_lstm_layer, self.valnet_output_layer)
                val_hat = tf.squeeze(val_hat, axis=-1)
                beam_score_list.append(val_hat)

            if len(beam_score_list)==0:
                top_decoded_ids = decoded_ids[:, 0, 0:]  # should include [begin], [batch, beam_size, length]
                top_scores = scores[:, 0]  # not used
                return {"outputs": top_decoded_ids, "scores": top_scores}

            beam_score = tf.stack(beam_score_list, axis=-1)
            beam_score = tf.math.reduce_mean(beam_score, axis=-1)
            output_list.append(beam_score)

        custom_score = tf.stack(output_list, axis=-1)
        adj_custom_score = scores+custom_score*1.0
        ranking = tf.argsort(adj_custom_score, axis=-1, direction='DESCENDING')
        ...
        if self.use_beam_filter == True:
            top_decoded_ids = tf.gather_nd(decoded_ids, indices)
            # top_decoded_ids = decoded_ids[:, -1, 0:]  # should include [begin], [batch, beam_size, length]
